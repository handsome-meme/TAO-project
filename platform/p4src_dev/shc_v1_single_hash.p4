...
struct shc_ingress_metadata_t {
    ...
    bit<1>  hash_size_done_flag; 
    bit<1>  threshold_done_flag;
    bit<1>  count_threshold_flag;
...
parser ShcIngressParser(
    ...
    state parse_inner_ethernet {
        ...
        ig_md.p_size = hdr.inner_ipv4.total_len;
               ...
               inout shc_ingress_metadata_t ig_md)()
{
    ...
    table count_threshold_table{
        actions = {
            count_threshold;
        }
        default_action = count_threshold();
        size = 1;
    }
    ...
    action get_hash_value_1(){
        ig_md.hash_value = hash1.get({ig_md.vni, ig_md.src_addr, ig_md.dst_addr});
    }
    table get_hash_value_table{ 
        ...
        size = 1;
        default_action = get_hash_value_1();
    }
    
    //hash1_1
    Register<bit<16>,bit<16>>(65536) hash1_1;
    RegisterAction<bit<16>,bit<16>,bit<16>>(hash1_1) size_update_1={
		void apply(inout bit<16> register_data, out bit<16> flow_size){
            register_data = register_data + hdr.inner_ipv4.total_len;
            flow_size = register_data;
		}
	};
    action update_size_1(){
        ig_md.hash_size = size_update_1.execute(ig_md.hash_value);
        ig_md.hash_size_done_flag = 1;
    }    
    table update_size_1_table{
        actions = {
            update_size_1;
        }
        size = 1;
        default_action = update_size_1();
    }


    action size_threshold_cmp(){
        ig_md.threshold = (bit<32>)ig_md.hash_size |-| ig_md.threshold;
    }
    table size_threshold_cmp_table{
        actions = {
            size_threshold_cmp;
        }
        size = 1;
        default_action = size_threshold_cmp();
    }
    ...
    RegisterAction<bit<32>,bit<16>,bit<1>>(hash1_3) vni_check_1={
		void apply(inout bit<32> register_data,out bit<1> o_vni){
            if(register_data == ig_md.vni){
                o_vni = 1;
            }
            else{
                o_vni = 0;
    ...
    action get_vni_1(){
        ig_md.key_vni_flag = vni_check_1.execute(ig_md.hash_value);
    }
    table get_vni_1_table{
        actions = {
            get_vni_1;
        }
        size = 1;
        default_action = get_vni_1();
    }

    action update_vni_1(){
        vni_update_1.execute(ig_md.hash_value);
    }
    ...
    RegisterAction<ip_pair_t,bit<16>,bit<1>>(hash1_2) ip_check_1={
		void apply(inout ip_pair_t register_data,out bit<1> o_addr){
			if(register_data.src_addr == ig_md.src_addr){
                if(register_data.dst_addr == ig_md.dst_addr){
                    o_addr = 1;
                }
                else{
                    o_addr = 0;
    ...
    action get_ip_1(){
        ig_md.key_ip_flag = ip_check_1.execute(ig_md.hash_value);
    }
    table get_ip_1_table{
        actions = {
            get_ip_1;
        }
        size = 1;
        default_action = get_ip_1();
    }

    action update_ip_1(){
        ip_update_1.execute(ig_md.hash_value);
    }
    ...
    apply {
        get_hash_value_table.apply(); // 0_0
        count_all_size_table.apply(); // 0

        if(ig_md.count_threshold_flag == 1){ //1
            update_size_1_table.apply(); // 1_1
            count_threshold_table.apply(); // 2

            if(ig_md.hash_size_done_flag == 1 && ig_md.threshold_done_flag == 1){ // 22
                size_threshold_cmp_table.apply(); // 3
                if(ig_md.threshold > 0){ //8
                    ig_md.size_flag = 1;
                }
                else{
                    ig_md.size_flag = 0;
                ...
                cond_rnd_table.apply(); // 33
                if(ig_md.cond_rnd_flag == 1){ // 44
                    cond_rnd_m_table.apply(); // 44
                    if(ig_md.hash_cond > 0){ // 55
                        ig_md.replace_flag = 1; // 7
                        update_ip_1_table.apply();// 66 主要问题应该是这里，注释了就没问题，如果和下面的table一起就会报错
                        update_vni_1_table.apply();// 55
                    }
                    else{
                        ig_md.replace_flag = 0; // 7
                        get_ip_1_table.apply();// 66 主要问题应该是这里，注释了就没问题，如果和下面的table一起就会报错
                        get_vni_1_table.apply();// 55

                        if(ig_md.key_vni_flag == 1 && ig_md.key_ip_flag ==1){ // 77
                            ig_md.key_flag = 1;
                        }
                        else{
                            ig_md.key_flag = 0;
                 ...
                 inout ingress_intrinsic_metadata_for_tm_t        ig_tm_md    )
{
    ...
    apply{
        ...
        if (hdr.vxlan.isValid() && hdr.inner_ipv4.isValid()) { // 88
            if(ig_md.size_flag == 1){ // size是否大于阈值
                if(eflow_forward.apply().hit){ // 9
                    nexthop.apply();  // 10
                    ig_tm_md.copy_to_cpu = 0;
                }
                else{
                    if(ig_md.replace_flag == 1){ // 是否替换key
                        ig_tm_md.copy_to_cpu = 1;
                    }
                    else if(ig_md.key_flag == 1){ // key是否相同
                        ig_tm_md.copy_to_cpu = 1;
                    }
                    else{
                        ig_tm_md.copy_to_cpu = 0;
