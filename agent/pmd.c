...
struct shc_mbuf_metadata {
    struct ip_tunnel_info in_tunnel_info;
    void *vrf_dp;
} __rte_cache_aligned;

static inline size_t
shc_mbuf_metadata_size(void)
{
    return sizeof(struct shc_mbuf_metadata);
}

static inline struct shc_mbuf_metadata *
shc_get_mbuf_priv(struct rte_mbuf *m)
{
    return rte_mbuf_to_priv(m);
}

static int
shc_dp_mbuf_init(shc_pmd_ctx_t *ctx)
{
    struct rte_mempool *mbuf_pool;

    mbuf_pool = rte_pktmbuf_pool_create("MBUF_POOL", NB_MBUF, 32,
                shc_mbuf_metadata_size(),
...
static int
shc_dp_mbuf_deinit(shc_pmd_ctx_t *ctx)
{
#if 0
    struct rte_mempool *mbuf_pool;
...
static pmd_thr_stat_t shc_pmd_thr_stat = PTS_INIT;

static inline int
shc_pmd_thr_stat_set(pmd_thr_stat_t stat)
{
    shc_pmd_thr_stat = stat;
    return 0;
}

static int
shc_dp_eal_init(shc_pmd_ctx_t *ctx)
{
...
//         if (localip == 0)
//             shc_dev_forward_vip_get(&localip);
                ...
                struct fib_res *f_res)
{
    ...
    if (ip_tunnel_inner_ether_need(&f_res->mpath[nhsel]->tunnel_info)) {
        ...
        if (!IP4_ADDR_IS_ZERO(f_res->mpath[nhsel]->gateway)) {
            if (shc_neigh_lookup(vrf_dp->vrf_id,
            ...
            rte_ether_addr_copy(shc_def_gw_mac_get(), &ethhdr->d_addr);
        }
...
{
    struct shc_mbuf_metadata *mbuf_md;
    vrf_dp_t *vrf_dp;
    struct rte_ipv4_hdr *iphdr;
    struct fib_res *f_res;
    int ret;

    mbuf_md = shc_get_mbuf_priv(pkt);
    vrf_dp = mbuf_md->vrf_dp; 
    RTE_ASSERT(vrf_dp != NULL);

    iphdr = rte_pktmbuf_mtod(pkt, struct rte_ipv4_hdr *);
    ret = shc_route_lookup(vrf_dp, iphdr->src_addr, iphdr->dst_addr, &f_res,0); //check vrf(route) table find the exact route path
    if (ret != 0) {
...
vxlan_tunnel_rcv(struct rte_mbuf *pkt, void *ctx)
{
    ...
    vrf_dp = shc_tunnel_map_vrf_lookup(SHC_TUNNEL_VXLAN, tunnel_id,0); //check tunnel map vrf table
    if (!vrf_dp) {
    ...
    mbuf_md = shc_get_mbuf_priv(pkt);
    mbuf_md->in_tunnel_info.type = SHC_ENCAP_VXLAN;
...
netio_rx(const int sock, void *arg)
{
    ...
    memset(shc_get_mbuf_priv(mbuf), 0, shc_mbuf_metadata_size());
...
#define SHC_PMD_THR_MAX   (8)
static shc_pmd_ctx_t *shc_pmd_ctxs[SHC_PMD_THR_MAX];

int
shc_pmd_thread_walk(pmd_thr_cb_t cb, void *arg1, void *arg2)
{
    int i;

    for (i = 0; i < RTE_DIM(shc_pmd_ctxs); i++)
        if (shc_pmd_ctxs[i])
            if (cb(shc_pmd_ctxs[i], arg1, arg2) < 0)
                break;

    return 0;
}

static shc_pmd_ctx_t *
shc_pmd_ctx_create(void)
{
    shc_pmd_ctx_t *pmd_ctx;
    int i, pid;

    /* Allocate a pmd ID.*/
    for (i = 0; i < RTE_DIM(shc_pmd_ctxs); i++)
        if (NULL == shc_pmd_ctxs[i])
            break;
    if (i >= RTE_DIM(shc_pmd_ctxs))
...
{
    shc_pmd_thread_args_t *thr_args = arg; //shc_api.h 全局变量

    prctl(PR_SET_NAME, "shc_pmd_thread");

    shc_pmd_ctx_t *pmd_ctx = shc_pmd_ctx_create();
    if (NULL == pmd_ctx)
...
{
    return PTS_RUNNING == shc_pmd_thr_stat;
}
