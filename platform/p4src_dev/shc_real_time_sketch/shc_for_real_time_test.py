...
def my_learning_cb(dev_id, pipe_id, direction, parser_id, session, msg):
    ...
    for digest in msg:
        ...
        if dtype == 1 : 
            print("port=%d dtype=%d vrf=%d src_ip=0x%08X dst_ip=0x%08X time_stamp=%d ------counting" % (port, dtype, vrf, src_ip, dst_ip, time_stamp))
        
        else : 
            print("port=%d dtype=%d vrf=%d src_ip=0x%08X dst_ip=0x%08X time_stamp=%d ------learning and insert" % (port, dtype, vrf, src_ip, dst_ip, time_stamp))
