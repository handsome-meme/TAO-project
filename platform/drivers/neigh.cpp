...
namespace shc
{
bool Neigh::mac_array_to_uint64(uint64_t& dst, const uint8_t from[6]) {
    dst = 0;
    uint64_t tmp;
...
bool Neigh::uint64_to_mac_array(uint8_t* dst, uint64_t from) {
    uint8_t byte = 0;
    byte = (int8_t)(from >> 40);
    dst[0] = byte;
    byte = (int8_t)(from >> 32);
...
void Neigh::KeySetUp(const hal_neigh_key &key,
                       bfrt::BfRtTableKey *table_key) { //将所有的key字段
  // Set value into the key object.
  bf_status_t bf_status = BF_SUCCESS;
  bf_status = table_key->setValue(neigh_id_field_id_,
                                  static_cast<uint64_t>(key.neigh_id));
  bf_sys_assert(bf_status == BF_SUCCESS);

  return;
}

void Neigh::DataSetUp(const hal_neigh_data &data,
                                  bfrt::BfRtTableData *table_data) { //将所有的data字段
  // Set value into the data object
  bf_status_t bf_status = BF_SUCCESS;
  uint64_t dmac;
  mac_array_to_uint64(dmac, data.dmac);
  bf_status = table_data->setValue(set_neigh_action_dmac_id_, dmac);
  bf_sys_assert(bf_status == BF_SUCCESS);

  return;
}

Neigh::Neigh(const bfrt::BfRtInfo *bfrtInfo, bf_rt_target_t& d_tgt, std::shared_ptr<bfrt::BfRtSession>& common_session){
  ...
  bf_status_t bf_status =
      bfrtInfo->bfrtTableFromNameGet("pipe.ShcIngress.neigh_tbl", &table_ptr_); //将指针table_ptr_指向硬件表
  bf_sys_assert(bf_status == BF_SUCCESS);

  // Get field-ids for key field 
  bf_status = table_ptr_->keyFieldIdGet("ig_md.meta.neigh_id",
                                          &neigh_id_field_id_); //对应到硬件表key字段
  bf_sys_assert(bf_status == BF_SUCCESS);

  // Get action Ids
  bf_status = table_ptr_->actionIdGet("ShcIngress.neigh",
                                        &set_neigh_action_id_); //对应到硬件表action字段
  bf_sys_assert(bf_status == BF_SUCCESS);

  // Get action data ids
  bf_status =
      table_ptr_->dataFieldIdGet("dmac",
                                   set_neigh_action_id_,
                                   &set_neigh_action_dmac_id_); //对应到硬件表相应action的data字段 vrf
  bf_sys_assert(bf_status == BF_SUCCESS);
...
hal_status_t Neigh::tableEntryAddModify(const hal_neigh_key *key, const hal_neigh_data *data, bool is_add){
//   hal_operation_t option = is_add ? HAL_ADD_OPERATION : HAL_MOD_OPERATION;
//   HAL_LOG_INFO(LOG_USER1, "recv Neigh {}. {},{} ", option, *key , *data);

  table_ptr_->keyReset(bfrtTableKey.get());

  table_ptr_->dataReset(set_neigh_action_id_, bfrtTableData.get());
...
hal_status_t Neigh::tableEntryGet(const hal_neigh_key *key, hal_neigh_data *data){
//   HAL_LOG_INFO(LOG_USER1, "recv Neigh {}. {} ", HAL_GET_OPERATION, *key);
  table_ptr_->keyReset(bfrtTableKey.get());
  ...
  uint64_t tmp;
  status = bfrtTableData->getValue(set_neigh_action_dmac_id_, &tmp);
  bf_sys_assert(status == BF_SUCCESS);
...
hal_status_t Neigh::tableEntryDel(const hal_neigh_key *key){
//   HAL_LOG_INFO(LOG_USER1, "recv Neigh {}. {} ", HAL_DEL_OPERATION, *key);
  table_ptr_->keyReset(bfrtTableKey.get());
  // Fill in the Key object
  KeySetUp(*key, bfrtTableKey.get());
...
hal_status_t Neigh::tableUsageGet(uint32_t *entry_count){
  bf_status_t status = BF_SUCCESS;
  uint64_t flags = 0;
...
hal_status_t Neigh::tableSizeGet(uint32_t *size){
  bf_status_t status = BF_SUCCESS;
  *size = size_;
  return status;
}

hal_status_t Neigh::AllocOid(uint32_t *oid) {
