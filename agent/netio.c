...
static rte_rwlock_t shc_dp_locker = RTE_RWLOCK_INITIALIZER;


void
shc_netio_write_lock(void)
{
    return rte_rwlock_write_lock(&shc_dp_locker);
}

void
shc_netio_write_unlock(void)
{
    return rte_rwlock_write_unlock(&shc_dp_locker);
}

void
shc_netio_read_lock(void)
{
    return rte_rwlock_read_lock(&shc_dp_locker);
}

void
shc_netio_read_unlock(void)
{
    return rte_rwlock_read_unlock(&shc_dp_locker);
}

static void
netio_cb(const int sock, short int which, void *arg)
{
    shc_pmd_ctx_t *ctx = arg;
...
static int
udp_netio_init(shc_pmd_ctx_t *ctx)
{
    const int on = 1;
    int raw_sock;
    int ret;
...
// static int
// gre_netio_init(shc_pmd_ctx_t *ctx)
// {
//     const int on = 1;
//     int raw_sock;
//     int ret;
