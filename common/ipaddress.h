...
public:
    ...
    inline bool isIpv6LinkLocal() const
    {
        if (m_ip.family != AF_INET6)
            return false;

        uint32_t ip6_msb;

        memcpy(&ip6_msb, &m_ip.ip_addr.ipv6_addr, sizeof(uint32_t));

        return (ip6_msb & htonl (0xffc00000)) == htonl (0xfe800000);
    }
    ...
    inline bool operator<(const IpAddress &o) const
    {
        ...
        if (m_ip.family == AF_INET)
            return htonl(m_ip.ip_addr.ipv4_addr) <
                   htonl(o.m_ip.ip_addr.ipv4_addr);
