...
namespace shc
{
void TunnelMapVrf::KeySetUp(const hal_tunnel_map_vrf_key &key,
                       bfrt::BfRtTableKey *table_key) { //将所有的key字段
  // Set value into the key object.
  bf_status_t bf_status = BF_SUCCESS;
  bf_status = table_key->setValue(tunnel_type_field_id_,
                                  static_cast<uint64_t>(key.tunnel_type));
  bf_sys_assert(bf_status == BF_SUCCESS);

  bf_status = table_key->setValue(tunnel_vni_field_id_,
                                  static_cast<uint64_t>(key.tunnel_key));
  bf_sys_assert(bf_status == BF_SUCCESS);

  return;
}

void TunnelMapVrf::DataSetUp(const hal_tunnel_map_vrf_data &data,
                                  bfrt::BfRtTableData *table_data) { //将所有的data字段
  // Set value into the data object
  bf_status_t bf_status = BF_SUCCESS;
  bf_status = table_data->setValue(set_tunnel_map_vrf_action_vrf_id_,
                                        static_cast<uint64_t>(data.vrf));
  bf_sys_assert(bf_status == BF_SUCCESS);
...
TunnelMapVrf::TunnelMapVrf(const bfrt::BfRtInfo *bfrtInfo, bf_rt_target_t& d_tgt, std::shared_ptr<bfrt::BfRtSession>& common_session){
  ...
  bf_status_t bf_status =
      bfrtInfo->bfrtTableFromNameGet("pipe.ShcIngress.tunnel_map_vrf_tbl", &table_ptr_); //将指针table_ptr_指向硬件表
  bf_sys_assert(bf_status == BF_SUCCESS);

  // Get field-ids for key field 
  bf_status = table_ptr_->keyFieldIdGet("ig_md.type",
                                          &tunnel_type_field_id_); //对应到硬件表key字段
  bf_sys_assert(bf_status == BF_SUCCESS);

  bf_status = table_ptr_->keyFieldIdGet("hdr.vxlan.vni", &tunnel_vni_field_id_); //对应到硬件表key字段
  bf_sys_assert(bf_status == BF_SUCCESS);

  // Get action Ids
  bf_status = table_ptr_->actionIdGet("ShcIngress.tunnel_map_vrf",
                                        &set_tunnel_map_vrf_action_id_); //对应到硬件表action字段
  bf_sys_assert(bf_status == BF_SUCCESS);

  // Get action data ids
  bf_status =
      table_ptr_->dataFieldIdGet("vrf",
                                   set_tunnel_map_vrf_action_id_,
                                   &set_tunnel_map_vrf_action_vrf_id_); //对应到硬件表相应action的data字段 vrf
  bf_sys_assert(bf_status == BF_SUCCESS);
...
hal_status_t TunnelMapVrf::tableEntryAddModify(const hal_tunnel_map_vrf_key *key, const hal_tunnel_map_vrf_data *data, bool is_add){
  // hal_operation_t option = is_add ? HAL_ADD_OPERATION : HAL_MOD_OPERATION;
  // HAL_LOG_INFO(LOG_USER1, "recv TunnelMapVrf {}. {},{} ", option, *key , *data);

  table_ptr_->keyReset(bfrtTableKey.get());

  table_ptr_->dataReset(set_tunnel_map_vrf_action_id_, bfrtTableData.get());
...
hal_status_t TunnelMapVrf::tableEntryGet(const hal_tunnel_map_vrf_key *key, hal_tunnel_map_vrf_data *data){
  // HAL_LOG_INFO(LOG_USER1, "recv TunnelMapVrf {}. {} ", HAL_GET_OPERATION, *key);
  table_ptr_->keyReset(bfrtTableKey.get());
  ...
  uint64_t tmp;
  status = bfrtTableData->getValue(set_tunnel_map_vrf_action_vrf_id_, &tmp);
  bf_sys_assert(status == BF_SUCCESS);
  data->vrf = static_cast<uint16_t>(tmp);

  return status;
}

hal_status_t TunnelMapVrf::tableEntryDel(const hal_tunnel_map_vrf_key *key){
  // HAL_LOG_INFO(LOG_USER1, "recv TunnelMapVrf {}. {} ", HAL_DEL_OPERATION, *key);
  table_ptr_->keyReset(bfrtTableKey.get());
  // Fill in the Key object
  KeySetUp(*key, bfrtTableKey.get());
...
hal_status_t TunnelMapVrf::tableUsageGet(uint32_t *entry_count){
  bf_status_t status = BF_SUCCESS;
  uint64_t flags = 0;
...
hal_status_t TunnelMapVrf::tableSizeGet(uint32_t *size){
  bf_status_t status = BF_SUCCESS;
  *size = size_;
  return status;
}
...
//                          const hal_tunnel_map_vrf_key &key) {
