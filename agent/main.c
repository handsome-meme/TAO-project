...
// extern void shc_topk_thread();
// extern void diff_cmp(void*);
...
{
    int option_index = 0;
    enum opts {OPT_SDEINSTALLDIR = 1, OPT_P4CONFFILE, OPT_BFSHELL,OPT_P4KERNELPKT, OPT_BFTMRCORE};
    static struct option options[] = {
        {"help", no_argument, 0, 'h'},
        {"sde_install-dir", required_argument, 0, OPT_SDEINSTALLDIR},
    ...
    while (1) {
        ...
        case '?':
            printf("%s\n", argv[0]);
            printf(
                "Usage : %s --sde_install-dir=<SDE install path> --p4-conf=<path to "
    ...
    if (switchd_ctx.install_dir == NULL) {
        printf("ERROR : --sde_install-dir must be specified\n");
        exit(0);
    }
...
int main(int argc, char **argv) 
{
    ...
    int ret;
    RTE_SET_USED(ret);
    shc_pmd_thread_args_t pmd_args;
    timer_para time_val;

    pthread_t thread_pmd_id;
    pthread_t thread_topk_id;

    RTE_SET_USED(thread_pmd_id);
    RTE_SET_USED(pmd_args);
    ...
    ret = pthread_create(&thread_pmd_id, NULL, shc_pmd_thread, (void*)(&pmd_args));
    if (ret) {
        rte_exit(EXIT_FAILURE, "Failed to create shc pmd thread\n");
    }
    printf("finish create shc pmd thread\n");
    time_val.interval_time = 5;
	time_val.func = diff_cmp;
    ret = pthread_create(&thread_topk_id, NULL, shc_topk_thread, (void*)(&time_val));
    if (ret) {
        rte_exit(EXIT_FAILURE, "Failed to create shc top k thread\n");
    }
    /* Give other threads some initialization time */
    sleep(2);

    printf("finish create shc top k thread\n");
    while (!shc_api_shc_pmd_thread_ready_get()) {
        sleep(1);
    }
    
    pthread_detach(thread_topk_id);
    // pthread_join(thread_topk_id, NULL);

    run_cli_or_cleanup();
    printf("finish run_cli_or_cleanup return\n");
}
