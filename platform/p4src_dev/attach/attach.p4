               ...
               inout shc_ingress_metadata_t ig_md)()
{
    ...
    Hash<hash_bucket_t>(HashAlgorithm_t.RANDOM) hash1;
    Hash<bit<16>>(HashAlgorithm_t.CRC16) hash2;

    action get_hash_value1(){
        ig_md.hash_value1 = hash1.get({ig_md.vni, ig_md.src_addr, ig_md.dst_addr}); //for test
    }
    ...
    action get_hash_value2(){
        ig_md.hash_value2 = hash2.get({ig_md.vni, ig_md.src_addr, ig_md.dst_addr}); //直接~ig_md.dst_addr, 会导致bfrt_python打不开
    }
    ...
    Register<bit<16>,hash_bucket_t>(1<<HASH_BUCKET_WIDTH) hash1_1;
    ...
    action update_pkt_num_1(){
        ...
        // ig_md.hash_pkt1_num = pkt_num_1.execute(1);
    }    
    ...
    action cond_rnd_m(){
        ig_md.hash_cond1 = PROB_NUM |-| ig_md.rnd1;
    }
        ...
        if(ig_md.cond_rnd_flag1 == 1){
            ...
            if(ig_md.hash_cond1 >= 0){ //for test =0
                ...
                if(ig_md.hash_pkt1_done == 1){
                    ...
                    ig_md.pkt_n_c = pkt_num_count.execute(0);
                    if(ig_md.hash_pkt1_num == HIT_COUNT){
                        ig_md.eflag = 1;
                    }
                    else{
                        ig_md.eflag = 0;
...
//for route selector
    ...
    table route_sele_tbl{
        key = {
            ig_md.vni : exact; //vrf change to vni
            hdr.inner_ipv4.src_addr : exact;
            hdr.inner_ipv4.dst_addr : exact;
        }
        actions = {
