...
// extern int
// shc_neigh_walk(neigh_entry_cb_t cb, void *arg1, void *arg2, void *arg3);
...
// extern int
// shc_arp_proxy_walk(struct ol_arp_proxy *arp_proxy, arp_proxy_entry_cb_t cb, void *arg1, void *arg2);
...
extern int
shc_api_neigh_ent_chg_notify(neigh_event_type_t type,
                const struct ol_neigh_lookup_key *key,
                const struct ol_neigh_entry *nhop);

/* ARP proxy.*/
// extern int
// shc_nd_proxy_lookup(const struct ol_arp_proxy *arp_proxy, 
//                 uint32_t proxy_ip, struct ol_arp_proxy_entry *entry);

// extern int
// shc_arp_proxy_add(struct ol_arp_proxy *arp_proxy, 
//                 const struct ol_arp_proxy_entry *entry);

// extern int
// shc_arp_proxy_del(struct ol_arp_proxy *arp_proxy, 
//                 uint32_t proxy_ip);

// extern int
// shc_arp_proxy_ent_cnt(const struct ol_arp_proxy *arp_proxy);

// extern int
// shc_arp_proxy_cleanup(struct ol_arp_proxy *arp_proxy);
...
struct rte_ether_addr *shc_hw_addr;
extern int
shc_neigh_lookup(uint32_t vrfid, uint32_t gw, struct rte_ether_addr *hw_addr,int flag_of_add_del);

// extern int
// shc_neigh_learning(shc_pmd_ctx_t *ctx,
//                 uint32_t vrfid, uint32_t gw,
//                 const struct rte_ether_addr *hw_addr,
//                 int notify);

// extern int
// shc_vrf_neigh_update(uint32_t vrfid);

// extern int
// shc_neigh_click_set(uint32_t vrfid, uint32_t ip);

extern int
shc_neigh_discover_timer(void *ctx);

// extern int
// shc_nd_sip_pick(const struct ol_arp_proxy *arp_proxy,
