...
namespace shc
{
  void RouteSele::KeySetUp(const hal_route_sele_key &key,
    ...
    bf_status = table_key->setValue(vni_field_id_,
                                    static_cast<uint64_t>(key.vni));
    bf_sys_assert(bf_status == BF_SUCCESS);
  ...
  void RouteSele::DataSetUp(const hal_route_data &data,
                                    bfrt::BfRtTableData *table_data) { //将所有的data字段
    // Set value into the data object
    bf_status_t bf_status = BF_SUCCESS;
    bf_status = table_data->setValue(set_route_sele_for_nexthop_action_nhop_id_id_,
                                          static_cast<uint64_t>(data.nhop_id));
    bf_sys_assert(bf_status == BF_SUCCESS);

    // bf_status = table_data->setValue(set_route_sele_for_nexthop_action_ttl_id_, entry_ttl);
    // bf_sys_assert(bf_status == BF_SUCCESS);
    return;
  }
  RouteSele::RouteSele(const bfrt::BfRtInfo *bfrtInfo, bf_rt_target_t& d_tgt, std::shared_ptr<bfrt::BfRtSession>& common_session){
    ...
    bf_status_t bf_status =
        bfrtInfo->bfrtTableFromNameGet("pipe.ShcIngress.route_sele_tbl", &table_ptr_); //将指针table_ptr_指向硬件表
    bf_sys_assert(bf_status == BF_SUCCESS);

    // Get field-ids for key field 
    bf_status = table_ptr_->keyFieldIdGet("ig_md.vni",
                                            &vni_field_id_); //对应到硬件表key字段
    bf_sys_assert(bf_status == BF_SUCCESS);
    ...
    // Get action Ids
    bf_status = table_ptr_->actionIdGet("ShcIngress.route_sele_for_nexthop",
                                          &set_route_sele_for_nexthop_action_id_); //对应到硬件表action字段
    bf_sys_assert(bf_status == BF_SUCCESS);

    bf_status = table_ptr_->actionIdGet("ShcIngress.route_sele_for_miss",
                                          &set_route_sele_for_miss_action_id_); //对应到硬件表action字段
    bf_sys_assert(bf_status == BF_SUCCESS);

    // Get action data ids
    bf_status =
        table_ptr_->dataFieldIdGet("nhop_id",
                                    set_route_sele_for_nexthop_action_id_,
                                    &set_route_sele_for_nexthop_action_nhop_id_id_); //对应到硬件表相应action的data字段 vrf
    bf_sys_assert(bf_status == BF_SUCCESS);

    bf_status =
        table_ptr_->dataFieldIdGet("$ENTRY_TTL",
                                    set_route_sele_for_nexthop_action_id_,
                                    &set_route_sele_for_nexthop_action_ttl_id_); //对应到硬件表相应action的data字段 vrf
    bf_sys_assert(bf_status == BF_SUCCESS);
  ...
  hal_status_t RouteSele::tableEntryAddModify(const hal_route_sele_key *key, const hal_route_data *data, bool is_add){
    // hal_operation_t option = is_add ? HAL_ADD_OPERATION : HAL_MOD_OPERATION;
    // HAL_LOG_INFO(LOG_USER1, "recv RouteSele {}. {},{} ", option, *key , *data);

    table_ptr_->keyReset(bfrtTableKey.get());

    table_ptr_->dataReset(set_route_sele_for_nexthop_action_id_, bfrtTableData.get());
  ...
  hal_status_t RouteSele::tableEntryGet(const hal_route_sele_key *key, hal_route_data *data){
    // HAL_LOG_INFO(LOG_USER1, "recv RouteSele {}. {} ", HAL_GET_OPERATION, *key);
    table_ptr_->keyReset(bfrtTableKey.get());
    ...
    uint64_t tmp;
    status = bfrtTableData->getValue(set_route_sele_for_nexthop_action_nhop_id_id_, &tmp);
    bf_sys_assert(status == BF_SUCCESS);
  ...
  hal_status_t RouteSele::tableEntryDel(const hal_route_sele_key *key){
    // HAL_LOG_INFO(LOG_USER1, "recv RouteSele {}. {} ", HAL_DEL_OPERATION, *key);
    table_ptr_->keyReset(bfrtTableKey.get());
    // Fill in the Key object
    KeySetUp(*key, bfrtTableKey.get());
  ...
  hal_status_t RouteSele::tableUsageGet(uint32_t *entry_count){
    bf_status_t status = BF_SUCCESS;
    uint64_t flags = 0;
  ...
  hal_status_t RouteSele::tableSizeGet(uint32_t *size){
    bf_status_t status = BF_SUCCESS;
    *size = size_;
    return status;
  }
